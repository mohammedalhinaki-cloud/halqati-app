...
export function buildPlan(student, settings) {
  // Any two surahs in ANY direction. Descending ranges (e.g. الناس -> الفاتحة) run the
  // engine in mirrored quarter-space p = TOTAL_Q - q, so progression stays monotonic
  // internally while every DISPLAY bound is converted back to real mushaf positions.
  const forward = Number(student.from) <= Number(student.to);
  const mapQ = (pos) => (forward ? pos : TOTAL_Q - pos);
  const startQ = forward ? rangeStartQ(student.from) : TOTAL_Q - rangeEndQ(student.from);
  const endQ = forward ? rangeEndQ(student.to) : TOTAL_Q - rangeStartQ(student.to);
  ...
  const step = (d, beyondPlan) => {
    ...
    const rFrom = mapQ(from);
    const rTo = mapQ(to);
    let qLo = Math.min(rFrom, rTo);
    let qHi = Math.max(rFrom, rTo);
    if (to === from) {
      // stalled/complete day: project full amount along the travel direction for readability
      const rProj = mapQ(Math.min(from + amt, endQ));
      qLo = Math.min(rFrom, rProj);
      qHi = Math.max(rFrom, rProj);
    }
    rows.push({
      type: 'day',
      date: d.iso,
