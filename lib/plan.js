...
/** "٢٠/٩" + Hijri small label */
export function dateLabels(isoStr) {
  const d = parseISO(isoStr);
  const g = `${d.getDate()}/${d.getMonth() + 1}`;
  let h = '';
  try {
    h = new Intl.DateTimeFormat('ar-SA-u-ca-islamic-umalqura-nu-arab', {
      day: 'numeric',
      month: 'long',
      year: 'numeric',
    }).format(d);
  } catch {}
  return { greg: g.replace(/\d+/g, (m) => m), day: d.getDate(), month: d.getMonth() + 1, year: d.getFullYear(), hijri: h };
}
