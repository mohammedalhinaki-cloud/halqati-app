...
export function buildPlan(student, settings) {
  ...
  const step = (d, beyondPlan) => {
    ...
    /* ---- review streams ----
       minor: yesterday's saved slice is QUEUED for today, but the estimate is
       never set automatically — the teacher must mark «تم» (like hifz). Unmarked
       slots keep showing their content (frozen) until marked; «لم تتم/غائب»
       burns the slot (plan slides). major: fully automatic descending ride over
       the WHOLE Quran starting at الناس — independent of the hifz range. */
    ...
    if (minorStatus === 'missed' || minorStatus === 'absent') {
      nMinor++; // burned: this stream slides one day only
      stats[minorStatus === 'missed' ? 'minorMissed' : 'minorAbsent']++;
    } else {
      const mslot = Math.max(0, dayIdx - nMinor);
      const it = minorFed[mslot] || null;
      const src = it || sliceQ(mslot > 0 ? mslot - 1 : null); // frozen preview of the scheduled flow
      if (src) {
        ...
        if (minorStatus === 'done') { stats.minorDone++; stats.minorSavedQ += q; }
      }
    }
    row.minor = minorCell;
    ...
    // feed: ONLY real saves queue tomorrow's minor review
